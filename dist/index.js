...
const codedeploy_1 = __importDefault(__nccwpck_require__(4599));
const iam_1 = __importDefault(__nccwpck_require__(58));
const sts_1 = __importDefault(__nccwpck_require__(7513));
async function run() {
    try {
        ...
        const codeDeploy = new codedeploy_1.default();
...
async function getAccountInformation() {
    ...
    const [{ AccountAliases: [awsAccountAlias = '?'] }, { Account: awsAccountId = '?' }] = await Promise.all([
        iam
            .listAccountAliases()
            .promise()
            .catch(() => {
...
Object.defineProperty(apiLoader.services['iam'], '2010-05-08', {
  get: function get() {
    var model = __nccwpck_require__(7041);
    model.paginators = (__nccwpck_require__(7583)/* .pagination */ .o);
    model.waiters = (__nccwpck_require__(7757)/* .waiters */ .V);
    return model;
  },
  enumerable: true,
  configurable: true
